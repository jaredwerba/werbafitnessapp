  ...
  return <>
    ...
    <div className="cols"><div>
      ...
      {S.routines.length ? <div className="list">{S.routines.map(r => <div key={r.id} className="item" onClick={() => nav('/plan/r/' + r.id)}>
        ...
        <div className="empty"><div className="ico"><Icon name="clipboard" /></div>{t('No routines yet.')}<br />{t('Build your own, or start from one of Nick’s programs.')}</div>
        <Button icon="sparkles" onClick={starterPlanSheet}>{t('Browse programs')}</Button>
      </>}
    </div></div>
  </>
}
